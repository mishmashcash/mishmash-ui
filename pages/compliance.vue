<template>
  <div class="compliance">
    <h1 class="title is-size-1 is-size-2-mobile is-spaced">
      MishMash.cash <span class="not-print">{{ $t('complianceTool') }}</span>
...
export default {
  ...
  methods: {
    ...
    print() {
      ...
      doc.setFontSize(20).text('MishMash.cash', padding, 40)
      doc.setFontStyle('normal').text(this.$t('complianceReport', 'en'), 70, 40)
      ...
      const logo = require('@/assets/img/mishmash_black.png')
      const imgLogo = new Image()
      imgLogo.src = logo
      doc.addImage(imgLogo, 'PNG', padding, padding, 58.4614, 11.1125)

      const markSize = 23.8125
      const mark = require('@/assets/img/mishmash_mark.png')
      const imgMark = new Image()
      imgMark.src = mark
      doc.addImage(imgMark, 'PNG', endX - markSize, 160, markSize, markSize)

      doc.save(
        `mishmash-compliance-${this.txDepositInfo.currency}-${this.txDepositInfo.amount}-${this.txDepositInfo.timestamp}.pdf`
      )
    },
    resetState() {
      this.loaded = false
