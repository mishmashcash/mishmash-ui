...
function getName({ path, netId, type, instance, format = '.json', currName = 'etn' }) {
...
function createTreeZip(netId) {
  try {
    ...
    const { instanceAddress: CONTRACTS } = config.tokens.etn
    ...
    for (const type of EVENTS) {
      for (const [instance] of Object.entries(CONTRACTS)) {
        const baseFilename = getName({
          ...
          format: '',
          path: TREES_PATH,
          currName: config.nativeCurrency.toLowerCase()
        })
...
async function createTree(netId) {
  try {
    ...
    const { instanceAddress: CONTRACTS } = tokens.etn
    ...
    for (const type of EVENTS) {
      for (const [instance] of Object.entries(CONTRACTS)) {
        ...
        let { events } = await loadZippedEvents({
          name: `${type}s_${netId}_${currName}_${instance}.json`,
          directory: EVENTS_PATH,
          deployedBlock
        })

        events = events.sort((a, b) => a.leafIndex - b.leafIndex)
