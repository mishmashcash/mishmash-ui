...
class EventService {
  constructor({ netId, amount, currency, factoryMethods }) {
    ...
    this.hasCache = this.isNative && (Number(this.netId) === 52014 || Number(this.netId) === 5201420)
  }
  ...
  async getEventsFromCache(type) {
    try {
      ...
      if (module) {
        const events = JSON.parse(module).sort((a, b) => a.blockNumber - b.blockNumber)
  ...
  dedupeEvents = (events) => {
    if (events && events.length) {
      ...
      events = events.filter((event) => {
        if (event.transactionHash && !seen.has(event.transactionHash)) {
  ...
  async getStatisticsRpc({ eventsCount }) {
    ...
    if (savedEvents && savedEvents.events.length) {
  ...
  async saveEvents({ events, lastBlock, type }) {
    try {
      ...
      console.error('saveEvents has error:', err.message, type, events)
    }
  }
}
