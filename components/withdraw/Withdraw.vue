  ...
  <b-tab-item :label="$t('withdraw')" header-class="button_tab_withdraw">
    <div class="field">
      <div class="label-with-buttons">
        ...
        >
          <b-tooltip
            :label="$t('depositTransactionOnExplorer')"
            size="is-small"
            position="is-left"
            multilined
          >
      ...
      async handler(withdrawNote) {
        ...
        try {
          ...
          if (!this.hasErrorNote) {
            const [mishmashPrefix, currency, amount, noteNetId, note] = this.withdrawNote.split('-')
            if (mishmashPrefix !== 'mishmash') {
              this.$store.dispatch('loading/disable')
              this.withdrawNote = `mishmash-${currency}-${amount}-${noteNetId}-${note}`
              return
            }
