...
class IndexedDB {
  ...
  async initDB() {
    try {
      ...
      this.db = await openDB(this.dbName, parseInt(process.env.DB_VERSION), this.options) // version (optional): Schema version, or undefined to open the current version.
      this.onEventHandler()
...
export default async (ctx, inject) => {
  ...
  const getInstance = (netId) => instances.get(`mishmash_${netId}`)
