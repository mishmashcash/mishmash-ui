...
export const getters = {
  ethProvider: (state, getters, rootState, rootGetters) => {
...
export const actions = {
  async askRelayerStatus(
    ...
    { hostname, relayerAddress, stakeBalance, name }
  ) {
    try {
      ...
      // check rewardAccount === relayerAddress for MASH burn, custom relayer - exception
      if (relayerAddress && response.data.rewardAccount !== relayerAddress) {
        throw new Error('The Relayer reward address must match registered address')
      }

      const isValid = validate(response.data)
      if (!isValid) {
        console.error('askRelayerStatus', hostname, relayerAddress, validate?.errors)
      ...
      console.error('askRelayerStatus', name, e.message)
  ...
  async pickRandomRelayer({ rootGetters, commit, dispatch, getters }) {
    ...
    const service = await relayerRegisterService(getters.ethProvider)
    const registeredRelayers = await service.getRelayers(netId)
  ...
  async getKnownRelayerData({ rootGetters, getters }, { relayerAddress, name }) {
    ...
    const service = await relayerRegisterService(getters.ethProvider)
    const [validRelayer] = await service.getValidRelayers(
  ...
  async relayTornadoWithdraw({ state, commit, dispatch, rootState }, { note }) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        dispatch('runJobWatcherWithNotifications', { id, type: 'mishmash', netId })
      } else {
