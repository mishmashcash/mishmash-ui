...
export const getters = {
  ...
  getArgsForOracle: (state, getters, rootState, rootGetters) => {
    const tokens = {
      ...networkConfig.netId52014.tokens,
      ...TOKENS
    }
...
export const actions = {
  fetchTokenPrice({ getters, commit, dispatch, rootState }) {
    if (getters.isPriceWatcherDisabled) {
      return
    }

    try {
      // const web3 = this.$provider.getWeb3(rootState.settings.netId52014.rpc.url)
      ...
      for (let i = 0; i < tokenAddresses.length; i++) {
        try {
          ...
          const price = 1000000000000000000 // await offchainOracle.methods.getRateToEth(tokenAddresses[i], isWrap).call()
