...
const getters = {
  ...
  govContract: (state, getters, rootState) => ({ netId }) => {
    ...
    const address = config['governance.contract.mishmash.cash']
    if (address) {
  ...
  isEnabledGovernance: (state, getters, rootState, rootGetters) => {
    return Boolean(rootGetters['metamask/networkConfig']['governance.contract.mishmash.cash'])
  },
  constants: (state) => {
...
const actions = {
  ...
  async castVote(context, payload) {
    ...
    try {
      ...
      const data =
        delegators.includes(ethAccount) && delegators.length === 1
          ? govInstance.methods.castVote(id, support).encodeABI()
          : govInstance.methods.castDelegatedVote(delegators, id, support).encodeABI()
  ...
  async checkActiveProposals({ getters, rootGetters, commit }) {
    ...
    const { 'governance.contract.mishmash.cash': governanceAddress } = rootGetters['metamask/networkConfig']
    const aggregatorContract = getters.aggregatorContract
