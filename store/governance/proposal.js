...
const parseComment = (calldata, govInstance) => {
  ...
  try {
    ...
    try {
      const str = utils.defaultAbiCoder.decode(
        ['string'],
        utils.hexDataSlice(calldata, utils.hexDataLength(data))
      )
      const [contact, message] = JSON.parse(str)
      return { contact, message }
    } catch {
      // Fallback to castVote
    }
  } catch {
    // Fallback to castVote
  }

  // Try castVote fallback
  try {
    const result = utils.defaultAbiCoder.decode(
      ['uint256', 'bool'],
      utils.hexDataSlice(calldata, methodLength)
    )
    const data = govInstance.methods.castVote(...result).encodeABI()
...
const actions = {
  ...
  async fetchEnsNames(context, { comments }) {
    ...
    try {
      ...
      console.log('fetchEnsNames', addresses)
