...
const state = () => {
  return {
    ...
    ip: {},
    selectedInstance: { currency: 'etn', amount: 1 },
    selectedStatistic: { currency: 'etn', amount: 1 },
...
const getters = {
  ...
  tornadoProxyContract: (state, getters, rootState) => ({ netId }) => {
    const {
      ...
      'mishmash-router': tornadoProxyLight
  ...
  hasEnabledLightProxy: (state, getters, rootState, rootGetters) => {
    return Boolean(rootGetters['metamask/networkConfig']['mishmash-light-router'])
  },
...
const actions = {
  ...
  async loadEncryptedEvents(_, { netId }) {
    try {
      ...
      throw new Error(`Method loadEncryptedEvents has error: ${err.message}`)
    }
  },
  ...
  async getEncryptedNotes({ rootState, rootGetters, dispatch, getters }) {
    try {
      ...
      console.log('dbCachedEvents', cachedEvents)

      const networksWithCache = {
        52014: cachedEventsLength.mainnet.ENCRYPTED_NOTES,
        5201420: cachedEventsLength.testnet.ENCRYPTED_NOTES
      }
  ...
  async createSnarkProof(
    ...
    if (withdrawType === 'relayer') {
      ...
      console.log('selectedRelayer', rootState.relayer.selectedRelayer)
